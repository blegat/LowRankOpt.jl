...
function maxcut(weights, solver)
    ...
    factors = LRO.TriangleVectorization.(LRO.positive_semidefinite_factorization.(e_i.(1:N, N)))
    set = LRO.SetDotProducts{LRO.WITH_SET}(cone, factors)
    @variable(model, dot_prod_set[1:length(factors)+MOI.dimension(cone)] in set)
    dot_prod = dot_prod_set[1:length(factors)]
    X = reshape_vector(dot_prod_set[length(factors).+(1:MOI.dimension(cone))], SymmetricMatrixShape(N))
