...
function errors(model::Model, x;
    ...
    C_den = 1 + LinearAlgebra.norm(NLPModels.grad(model, ScalarIndex), 1) + sum(matrix_indices(model), init = zero(b_den)) do i
        LinearAlgebra.norm(NLPModels.grad(model, i), 1)
    end
    obj_den = 1 + abs(pobj) + abs(dobj)
    (
        LinearAlgebra.norm(primal_err) / b_den,
        max(0, -LinearAlgebra.eigmin(x)) / b_den,
        isnothing(dual_err) ? zero(b_den) : LinearAlgebra.norm(dual_err) / C_den,
        isnothing(dual_slack) ? zero(b_den) : LinearAlgebra.norm(dual_err) / C_den,
        (pobj - dobj) / obj_den,
...
function LinearAlgebra.eigmin(x::AbstractSolution{T}) where {T}
    return min(minimum(x[ScalarIndex], init = zero(T)) + minimum(matrix_indices(x), init = zero(T)) do i
        LinearAlgebra.eigmin(LinearAlgebra.Symmetric(x[i]))
    end)
end
